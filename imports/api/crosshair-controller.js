...
class Crosshair {
  ...
  regEvents() {
    EventManager.addAll([
      {name:'crosshair:left',call: ()=> this.move(this.LEFT)},
      {name:'crosshair:right',call:  ()=> this.move(this.RIGHT)},
      {name:'crosshair:up',call:  ()=> this.move(this.UP)},
      {name:'crosshair:down',call: ()=> this.move(this.DOWN)}
    ]);
  }
  //TODO: REMOVE css...
  move(direction) {
  ...
  //SETTER & GETTER
  set obj(obj=def) {
    var {id ,url,width,startpos,goal} = obj;
    this.id=id;
    this.url = url;
    this.width = width;
